 ...
 * missing (malformed response) or if the granted `scope` doesn't include
 * `GMAIL_SCOPE` (`"insufficient_scope"` — the user unticked the Gmail
 * checkbox on Google's granular consent screen). Network failures from
 * `fetchImpl` propagate as-is.
 */
async function requestTokenSet(
  ...
  if (!scope.split(" ").includes(GMAIL_SCOPE)) {
    throw new OAuthError(
      "insufficient_scope",
      "Granted scope did not include Gmail access — the Gmail checkbox on " +
        "Google's consent screen may have been unticked.",
    );
  }
 ...
 * (`grant_type=authorization_code`), completing the PKCE flow.
 */
export function exchangeCode(opts: ExchangeCodeOptions): Promise<TokenSet> {
  ...
  return requestTokenSet(opts.fetch, opts.now, body);
}
 ...
 * token they already have unless a new one is returned.
 */
